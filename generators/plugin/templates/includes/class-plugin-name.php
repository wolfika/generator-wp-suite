...
class <%= plugin.name.className %> {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      <%= plugin.name.className %>_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct() {
		...
		$this->plugin_name = '<%= plugin.name.fileName %>';
		$this->version = '1.0.0';
	...
	private function load_dependencies() {
		 ...
		 * core plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-<%= plugin.name.fileName %>-loader.php';
	 ...
	 *
	 * Uses the <%= plugin.name.className %>_i18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0.0
	 * @access   private
	...
	private function set_locale() {
		...
		$plugin_i18n = new <%= plugin.name.className %>_i18n();
	...
	private function define_admin_hooks() {
		...
		$plugin_admin = new <%= plugin.name.className %>_Admin( $this->get_plugin_name(), $this->get_version() );
	...
	private function define_public_hooks() {
		...
		$plugin_public = new <%= plugin.name.className %>_Public( $this->get_plugin_name(), $this->get_version() );
	 ...
	 *
	 * @since     1.0.0
	 * @return    <%= plugin.name.className %>_Loader    Orchestrates the hooks of the plugin.
	 */
